...
def write_file(path: str, content: dict[str, Any], sha: str | None, commit_msg: str) -> None:
    body: dict[str, str] = {
        "message": commit_msg,
...
def main():
    ...
    closers      : list[str]   = mod_config.get("closers", [])
    strikers : list[str] = mod_config.get("strikers", [])
    moderators   : list[str]   = mod_config.get("moderators", [])
    ...
    if EVENT == "discussion_comment":
        ...
        # /ban @user <reason>
            ...
            reason_parts: list[str] = COMMENT_BODY.split(None, 2)
            ...
            reason: str = reason_parts[2].strip()
